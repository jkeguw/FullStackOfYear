...
# Install ts-morph for advanced TypeScript transformations
echo "Installing ts-morph for advanced TypeScript fixes..."
cd frontend
sudo npm install --save-dev ts-morph
cd ..

# Create ts-morph script file
cat > ./scripts/fix-with-ts-morph.js << 'EOF'
...
function fixMissingImports() {
  ...
  const elMessageFiles = project.getSourceFiles().filter(
    file => file.getFullText().includes('ElMessage') && 
           !file.getImportDeclarations().some(importDecl => 
             importDecl.getModuleSpecifierValue().includes('element-plus') && 
...
echo "ts-morph installation and script creation complete!"
