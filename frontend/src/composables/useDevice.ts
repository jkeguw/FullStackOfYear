...
interface UserDeviceListParams {
  userId?: string;
  page?: number;
  pageSize?: number;
  isPublic?: boolean;
...
export function useDevice() {
  ...
  const currentUserDevice = ref<UserDevice | null>(null);
  const userDeviceLoading = ref(false);
  const userDeviceError = ref<string | null>(null);
  const userDevicePagination = reactive({
    total: 0,
    page: 1,
    pageSize: 9
  });
  ...
  // 用户设备配置相关方法
  // 注意：这些方法保留但已不再使用，相关功能已从后端移除
  // 获取用户设备列表
  const fetchUserDevices = async (params?: UserDeviceListParams) => {
    ElMessage.info('个人设备管理功能已被移除，请使用鼠标比较功能');
    return null;
  };
