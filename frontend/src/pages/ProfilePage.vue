      ...
      <el-tabs v-model="activeTab" class="profile-tabs" @tab-click="handleTabClick">
        <el-tab-pane label="个人资料" name="profile">
          <div class="mx-auto max-w-3xl">
            <ProfileTab :loading="profileLoading" :profile="profile" @update="updateUserProfile" />
          </div>
        </el-tab-pane>
...
onMounted(() => {
  ...
  if (
    tab &&
    ['profile', 'game-settings', 'settings', 'privacy', 'security', 'devices'].includes(tab)
  ) {
    activeTab.value = tab;
  }
