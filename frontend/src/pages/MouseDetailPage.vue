  ...
  <div class="mouse-detail-page">
    <div class="container mx-auto py-6 px-4">
          ...
          <div class="md:w-1/2">
            <div class="mouse-image-container bg-white rounded-lg p-6 mb-4 shadow-sm">
              <div class="relative aspect-video mb-2">
                <div
                  v-if="activeImage.type === 'svg'"
                  class="w-full h-full svg-container"
                >{{ activeImage.content }}</div>
              ...
              <div class="flex gap-2 mt-4">
                ...
                <el-button type="success" size="small" @click="addToComparison">
            ...
            <div class="mouse-dimensions bg-white rounded-lg p-6 shadow-sm">
              ...
              <div class="dimensions-grid">
                ...
                <div v-if="mouse.dimensions.gripWidth" class="dimension-item">
          ...
          <div class="md:w-1/2">
            <div class="bg-white rounded-lg p-6 shadow-sm">
              <div class="flex items-center mb-6">
                ...
                <div
                  v-if="isInComparison"
                  class="badge bg-success-100 text-success-700 px-2 py-1 rounded"
                >
                  已加入比较
                </div>
              </div>

              <div v-if="mouse.description" class="mb-6">
                      ...
                      <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="spec-item">
                          ...
                          <div class="spec-value">
                            {{ formatHumpPlacement(mouse.shape.humpPlacement) }}
                          </div>
                        </div>
                        <div class="spec-item">
                        ...
                        <div class="spec-item">
                          ...
                          <div class="spec-value">
                            {{ formatHandCompatibility(mouse.shape.handCompatibility) }}
                          </div>
                        </div>
                        <div v-if="mouse.shape.thumbRest !== undefined" class="spec-item">
                          <div class="spec-label">拇指区</div>
                          <div class="spec-value">
                            {{ mouse.shape.thumbRest ? '有拇指托' : '无拇指托' }}
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="spec-group mt-6">
                      ...
                      <div class="grid grid-cols-2 gap-4">
                        <div class="spec-item">
                          ...
                          <div class="spec-value">
                            {{ formatConnectivity(mouse.technical.connectivity) }}
                          </div>
                        </div>
                        <div class="spec-item">
                        ...
                        <div v-if="mouse.technical.battery" class="spec-item">
                  ...
                  <div class="recommendations">
                    <div class="mb-4">
                      ...
                      <div class="flex gap-2 flex-wrap">
                        <el-tag
                          v-for="game in mouse.recommended.gameTypes"
                          :key="game"
                          size="small"
                        >
                          {{ game }}
                    ...
                    <div class="other-recommendations grid grid-cols-2 gap-4 mt-6">
                      <div class="recommendation-item">
                        ...
                        <div class="flex items-center">
                          <el-icon
                            v-if="mouse.recommended.dailyUse"
                            class="text-success"
                            size="large"
                          >
                            <CircleCheckFilled />
                          ...
                          <span class="ml-2">{{
                            mouse.recommended.dailyUse ? '适合' : '不适合'
                          }}</span>
                        </div>
                      </div>
                      ...
                      <div class="recommendation-item">
                        ...
                        <div class="flex items-center">
                          <el-icon
                            v-if="mouse.recommended.professional"
                            class="text-success"
                            size="large"
                          >
                            <CircleCheckFilled />
                          ...
                          <span class="ml-2">{{
                            mouse.recommended.professional ? '适合' : '不适合'
                          }}</span>
                        </div>
                      </div>
                    </div>
                          ...
                          <div class="flex items-center">
                            <div class="mouse-image w-16 h-16 mr-3 flex-shrink-0">
                              <template v-if="similarMouse.imageUrl">
                                <img
                                  :src="similarMouse.imageUrl"
                                  class="w-full h-full object-contain"
                                  :alt="similarMouse.name"
                                />
                              </template>
                              <template v-else-if="similarMouse.svgData?.topView">
                                <div
                                  class="w-full h-full svg-container"
                                >{{ similarMouse.svgData.topView }}</div>
                              </template>
                              <template v-else>
                                <div
                            ...
                            <div>
                              <h3 class="text-base font-medium">
                                {{ similarMouse.brand }} {{ similarMouse.name }}
                              </h3>
                              <p class="text-xs text-gray-500 mt-1">
                                {{ similarMouse.dimensions.length }}×{{
                                  similarMouse.dimensions.width
                                }}×{{ similarMouse.dimensions.height }}mm
                              </p>
      ...
      <div v-else class="error-container text-center py-20">
        ...
        >
          <template #extra>
            <el-button type="primary" @click="$router.push({ name: 'DeviceList' })"
              >返回鼠标列表</el-button
            >
          </template>
        </el-result>
...
import { getDevices, getMouseDevice } from '@/api/device';
...
async function fetchMouseData() {
  ...
  try {
    // 从API获取鼠标信息
    const response = await getMouseDevice(mouseId.value);
    if (response && response.data) {
      mouse.value = response.data;
      comparisonStore.addToRecentlyViewed(response.data);
    } else {
      mouse.value = null;
    }
  } catch (error) {
...
async function fetchAllMice() {
  try {
    ...
    allMice.value = response.data.devices;
    findSimilarMice();
  } catch (error) {
...
function findSimilarMice() {
  ...
  // 使用比较服务找出相似鼠标
  similarMice.value = comparisonService.findSimilarMice(mouse.value, allMice.value, 3);
}
